          ...
          int iMaxRunTimes, int iNodeId, char *pczLogicName )
{
   ...
   char aczBuffer[65536], *pczBuffer;
   apr_size_t tLen;
   int iInteractFlag = 0;
   /**
     * add for dynamic change config shm
   ...
   while( pstProcess->m_bActive && iRunTimes < iAllowRunTimes )
   {
      ...
      pstClient = pstListener;
      /* must be BLOCK  , must not close *
...
int main( int argc, char *argv[] )
{
   ...
   *********/
   ...
   stSockInfo.type    = SOCK_DGRAM;
   stSockInfo.protocol  = APR_PROTO_UDP;
