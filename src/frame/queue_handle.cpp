...
int iInQueId, iOutQueId, iCleanQueId;
...
int main( int argc, char *argv[] )
{
   ...
   char *pczTemp;
   int iInQueKey, iOutQueKey, iCleanQueKey;
   ...
   ******/
   ...
   if (iOutQueId < 0) 
   {
       fprintf (stderr, "获取交易响应队列失败! [%s] Key=[%s]\n",
                strerror (errno), iOutQueKey);
       return -1;
   }

   
  // LOG4C((LOG_DEBUG, "main iOutQueId.[%d]\n", iOutQueId));

   pczTemp = getenv ("CLEAN_QUE_KEY");
   if (pczTemp != NULL)
   {
       iCleanQueKey = atoi (pczTemp);
       iCleanQueId = msgget (iCleanQueKey, IPC_CREAT | 0660);
       if (iCleanQueId < 0) 
       {
           fprintf (stderr, "获取清除堵塞交易队列失败! [%s] Key=[%s]\n",
                    strerror (errno), iCleanQueKey);
           return -1;
       }
   }
   else
   {
       LOG4C(( LOG_WARN, "未定义清除堵塞消息队列CLEAN_QUE_KEY" ));
       iCleanQueId = -1;
   }
