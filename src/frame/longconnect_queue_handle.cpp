          ...
          int iMaxRunTimes, int iNodeId, char *pczExecFile )
{
   ...
   apr_pool_create( &pstSocketPool, gpstRootPool );
...
int main( int argc, char *argv[] )
{
   ...
   *********/
       ...
       APR_SUCCESS )
   {
    ...
    apr_socket_opt_set(pstListener, APR_SO_REUSEADDR,1);

 #if 0  
   
   if(apr_socket_opt_set( pstListener, APR_SO_NONBLOCK, 1 ) != APR_SUCCESS)
   {
      LOG4C(( LOG_FATAL, "failed to apr_socket_opt_set APR_SO_NONBLOCK %s\n",
              strerror(errno) ));
      return -1;
   }
   if(apr_socket_opt_set( pstListener, APR_SO_KEEPALIVE, 1 )!= APR_SUCCESS )
   {
