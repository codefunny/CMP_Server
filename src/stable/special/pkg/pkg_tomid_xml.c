...
printf ("[%d]\n", iSeq);
      if( iSeq != PKG_REP_SYMBOL )  /* 非重复域 */ 
      {
         ...
         if (iNestedLevel != 0)
         {
             ...
             if( iRet != PKG_SUCC )
             {
            LOG4C ((LOG_FATAL, "PKG_GetXmlItem "
                        "failed, err code[%d]",  iRet ));
                 continue;
             }
 ...
 /* ///////////////////////////////////////////////// */
            ...
            if( stMsgFldCfg.iFldType == UDT_STRING   )
            {
               int iTmp = 0;
               int iFldLen = 0;
               char aczFldDef[5];
/*
                LOG4C ((LOG_DEBUG, "stMsgFldCfg.aczFldDef[%s] ",stMsgFldCfg.aczFldDef));
*/
               memset(aczFldDef,0x00,sizeof(aczFldDef));
               PKG_XToA(stMsgFldCfg.aczFldDef ,aczFldDef);
               iFldLen = iLen ;
               switch( stMsgFldCfg.aczFldAlign[0] )
               {
                  case 'L':
                  case 'l':
                     while( (aczValue + iFldLen -iTmp-1)[0] == aczFldDef[0] 
                            && iTmp < iFldLen )
                     {
                        iTmp++;
                     }
                     ...
                     break;
                  default:
                     while( (aczValue + iTmp)[0] == aczFldDef[0]  
                            && iTmp < iFldLen ) 
                     {
                        iTmp++;
                     }
                      iLen = iFldLen - iTmp;
                      iRet = UDO_AddField( opfsAddr, iSeq, iFldType, aczValue  + iTmp, iLen );
                      if( iRet != PKG_SUCC )
                      {
                         LOG4C ((LOG_FATAL,
                                 "PKG_AddFldContentToLST "
      ...
      else /* 重复域 */
      {
         ...
         for( x = 1; x <= iRepFldNums; x++ )
         {
            ...
            if (iRepSeq >= NESTED_XML_BEGINTAG_E && iRepSeq <= NESTED_XML_BEGINTAG_S)
            {
                  iNestedLevel += 1;
                  continue;
            }
            if (iRepSeq >= NESTED_XML_ENDTAG_E && iRepSeq <= NESTED_XML_ENDTAG_S)
            {
                  iNestedLevel -= 1;
                  continue;
            }
            ...
            while (iMoveLen_xml  < eiOrgMsgLen)
            {
               ...
               {
               LOG4C ((LOG_FATAL, "PKG_GetXmlItem"
                           " failed, err code[%d]iRepTimes[%d]",  iRet ,iRepTimes));
                   break;
               }
            ...
            if( stMsgFldCfg.iFldType == UDT_STRING   )
            {
               ...
               switch( stMsgFldCfg.aczFldAlign[0] )
               {
                  ...
                  case 'l':
                     ...
                     while( (aczValue + iLen -iTmp-1)[0] == aczFldDef[0] 
                            && iTmp < iLen )
                     {
                        iTmp++;
                     }
                      iLen = iLen - iTmp;
/* ////////////////////////////////////
                LOG4C ((LOG_DEBUG, "iRepSeq[%d]aczValue[%s]iLen[%d]iFldType[%d]iTmp[%d] ",iRepSeq,aczValue ,iLen,iFldType,iTmp ));
*/
                      iRet = UDO_AddField( opfsAddr, iRepSeq, iFldType, aczValue, iLen );
                      if( iRet != PKG_SUCC )
                      {
                         LOG4C ((LOG_FATAL,
                                 "PKG_AddFldContentToLST "
...
/* ////////////////////////////////////
                ...
                LOG4C ((LOG_DEBUG, "iRepSeq[%d]aczValue[%s] ",iRepSeq,aczValue  + iTmp));
*/
                      iRet = UDO_AddField( opfsAddr, iRepSeq, iFldType, aczValue  + iTmp, iLen );
                      if( iRet != PKG_SUCC )
                      {
                         LOG4C ((LOG_FATAL,
                                 "PKG_AddFldContentToLST "
