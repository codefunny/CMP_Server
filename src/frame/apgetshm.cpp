...
int CommandServer::start( apr_socket_t *pstClient, char *pczName )
{
   ...
   if( getRunningIndex( pczName ) != -1 )
   {
      ...
      sprintf( aczRsp, "%s 子系统已经在运行", pczName );
      tLen = strlen( aczRsp );
   ...
   sprintf( aczRsp, "%s 子系统已经开始运行", pczName );
   tLen = strlen( aczRsp );
   apr_socket_send( pstClient, aczRsp, &tLen );
...
int CommandServer::stop( apr_socket_t *pstClient, char *pczName )
{
   ...
   sprintf( aczInfo, "停子系统%s成功", pczName );
   tLen = strlen( aczInfo );
   apr_socket_send( pstClient, aczInfo, &tLen );
...
ProcessRegister* m_pstProcess2;
SystemRegister * m_pstSystem2;

   if(argc < 6 )
   {
      printf("\nusage :%s shmkey maxpro index count  maxsys\n" ,argv[0]);
      return 0 ;
   } 
   if( fork() > 0 )
   {
   ...
   iSize = sizeof( ProcessRegister ) * atoi(argv[2]) * atoi(argv[3]);
   LOG4C(( LOG_DEBUG, "计算出的进程管理共享内存大小 %d\n", iSize ));
   m_pstProcess2 = (ProcessRegister*)shm_get( atoi(argv[1]), iSize );
   if( m_pstProcess2 == NULL )
   {
      LOG4C(( LOG_DEBUG, "创建共享内存(KEY=%d)失败",
               atoi(argv[1])  ));
      return -1;
   }
   memset(aczFileN,0,sizeof(aczFileN));
   sprintf(aczFileN,"%s/log/getshm.log",getenv("MYROOT"));
   fp = fopen(aczFileN,"a");
   for ( i=0; i< atoi(argv[4]) ; i++ )
   {
       fprintf(fp,"[%d]m_bActive[%d]\n",i,m_pstProcess2->m_bActive);
       fprintf(fp,"[%d]m_bDoTxn[%d]\n",i,m_pstProcess2->m_bDoTxn);
       fprintf(fp,"[%d]m_tPid[%d]\n",i,m_pstProcess2->m_tPid);
       fprintf(fp,"[%d]m_iRunTimes[%d]\n",i,m_pstProcess2->m_iRunTimes);
       fprintf(fp,"[%d]m_tBegin[%d]\n",i,m_pstProcess2->m_tBegin);
       fprintf(fp,"\n" );
       m_pstProcess2 ++;
   }
   fflush(fp);
   ...
   iSize = sizeof(SystemRegister) * atoi(argv[5]) ;
   LOG4C(( LOG_DEBUG, "计算出的系统管理共享内存大小 %d\n", iSize ));
   m_pstSystem2 = (SystemRegister*)shm_get( atoi(argv[1]) -1 , iSize );
   if( m_pstSystem2 == NULL )
   {
      LOG4C(( LOG_DEBUG, "创建共享内存(KEY=%d)失败",  atoi(argv[2]) -1 ));
      return -1;
   }
   ...
   {
       fprintf(fp,"[%d]m_bActive[%d]\n",i,m_pstSystem2->m_bActive);
       fprintf(fp,"[%d]m_aczName[%s]\n",i,m_pstSystem2->m_aczName);
       fprintf(fp,"[%d]m_iPort[%d]\n",i,m_pstSystem2->m_iPort);
       fprintf(fp,"[%d]m_iMinProcess[%d]\n",i,m_pstSystem2->m_iMinProcess);
       fprintf(fp,"[%d]m_iMaxProcess[%d]\n",i,m_pstSystem2->m_iMaxProcess);
       fprintf(fp,"[%d]m_iActiveProcess[%d]\n",i,m_pstSystem2->m_iActiveProcess);
       fprintf(fp,"\n" );
       m_pstSystem2++;
   }
   fflush(fp);
